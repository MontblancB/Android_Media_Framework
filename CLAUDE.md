...
코드 수정 및 개선 작업이 완료되면 다음 프로세스를 따라 변경사항을 커밋하고 메인 브랜치에 푸시합니다.
...
1. **작은 단위로 자주 커밋**: 한 번에 여러 기능을 수정하지 말고, 논리적 단위로 분리
2. **의미 있는 커밋 메시지**: "수정", "업데이트" 같은 모호한 메시지 지양
3. **푸시 전 검토**: `git diff`, `git status`로 변경사항 확인
4. **테스트 후 커밋**: 로컬에서 정상 동작 확인 후 커밋
5. **민감 정보 제외**: API 키, 비밀번호 등은 절대 커밋하지 않음 (.gitignore 활용)
